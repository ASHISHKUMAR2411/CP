...
LinkedList *ReverseEveryAlternateKNodes(LinkedList *head, int k);
...
int main()
{
    ...
    while (1)
    {
        ...
        cout << "9. Reverse Every alternate Kth Node " << endl;
        cout << "10. Exit " << endl;
        ...
        case 9:
            cout << "enter the value of K part which you want to swap : ";
            cin >> k;
            Head = ReverseEveryAlternateKNodes(Head, k);
            display(Head);
            break;
        case 10:
            exit(0);
...
LinkedList *ReversePartUsingStack(LinkedList *head, int k)
{
    ...
    if(LengthList(curr) < k) return head ; 
    prev->link = NULL;
    return head;
}
LinkedList *ReverseEveryAlternateKNodes(LinkedList *head, int k)
{
    ...
    if(curr!=NULL)
    curr->link = ReverseEveryAlternateKNodes(curr->link,k);
    return prev;
}
LinkedList* deleteDuplicates(LinkedList* head)
{
    ...
    while(head && head->link && head->data == (head->link)->data)
    {
        head = (head->link)->link;
    }
    ...
    while(curr)
    {
        while(curr->link && (curr->data == (curr->link)->data))
            curr = (curr->link)->link;
        curr = curr->link;;
    }
    return head;
