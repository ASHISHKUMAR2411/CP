...
void segregrate_odd_even_node(struct ll **ptr)
{
    ...
    // in this method we try to mke two linked list for even and odd and then join them at the end

    // this mwthods takes extra space so not that useful 
    struct ll *evenS=NULL,*evenE=NULL,*oddS=NULL,*oddE=NULL;
    struct ll *curr=*ptr;
    while(curr!=NULL)
    {
        int val=curr->data;
