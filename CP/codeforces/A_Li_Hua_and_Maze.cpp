...
int binarySearch(vector<int> v, int To_Find)
{
    ...
    if (v[lo] == To_Find)
    {
        cout << "Found" << " At Index " << lo << endl;
    }
    else if (v[hi] == To_Find)
    {
        cout << "Found" << " At Index " << hi << endl;
    }
    else
    {
        cout << "Not Found" << endl;
...
ll mini(ll x1, ll y1, ll x2, ll y2, ll n, ll m){
    int cnt1 = 0;
    int cnt2 = 0;
...
int main()
{
    ...
    while (t--)
    {
        ...
        ll x1, x2, y1, y2;
        cin >> x1 >> y1 >> x2 >> y2;
        cout << mini(x1, y1, x2, y2, n, m) << endl;
    }

    return 0;
}
// (min(a, b)) + max(a, b) % min(a, b) + max(a, b) / min(a, b)