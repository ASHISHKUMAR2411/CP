...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < m - 1; i++)
        {
            if ((mp[arr[i]] >= mp[arr[i + 1]]) || (mp[arr[i + 1]] > (mp[arr[i]] + d)))
            {
                flag = true;
                break;
            }
            ans = min(ans, mp[arr[i + 1]] - mp[arr[i]]);
            if ((d + 1 - (mp[arr[i + 1]] - mp[arr[i]])) <= (n - mp[arr[i + 1]] + mp[arr[i]] - 1))
            {
                ans = min(ans, d + 1 - (mp[arr[i + 1]] - mp[arr[i]]));
            }
        }
        if (flag)
        {
