...
struct BigInt
{
    ...
    string to_string() const
    {
        ...
        else
        {
            ...
            while (s.size() and s.back() == base_char)
                s.pop_back();
            if (sign < 0)
                s += '-';
    ...
    BigInt &operator++()
    {
        sign > 0 ? increment() : decrement();
        return *this;
    }
    BigInt &operator--()
    {
        sign > 0 ? decrement() : increment();
        return *this;
    }
    BigInt operator++(int)
    {
    ...
    BigInt operator*(const BigInt &o) const
    {
        ...
        _sz |= _sz >> 8;
        _sz |= _sz >> 16;
        _sz++; // round up to next power of 2 >= size
        U.resize(_sz);
        V.resize(_sz);
