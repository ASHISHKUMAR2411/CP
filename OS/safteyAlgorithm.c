...
// Creating a structure to store the max_resources and required or needed resources and already allocated resources
struct process_info
{
    int max[10];
    int allocated[10];
...
// variables to store the number of processes and resources in the system
int no_of_process, no_of_resources;

// Take the input of the processes and resources from the user
void input(struct process_info process[no_of_process], int available[no_of_resources])
{
    // Fill array of Structure for the processes
    for (int i = 0; i < no_of_process; i++)
    {
        ...
        {
            printf("Enter max of [%d] : ", j + 1);
            scanf("%d", &process[i].max[j]);
        }
        printf("Enter No. of Allocated Resources for this process: ");
        for (int j = 0; j < no_of_resources; j++)
        {
            printf("Enter allocated of [%d] : ", j + 1);
            scanf("%d", &process[i].allocated[j]);

            // calculating the need/future need and storing it in need section of the structure
    ...
    {
        printf("Enter available of [%d] : ", i + 1);
        scanf("%d", &available[i]);
    }
}

// Printing the Info in Tabular Format
